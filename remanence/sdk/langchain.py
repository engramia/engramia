"""LangChain integration for Remanence.
...
class RemanenceCallback:
    """LangChain callback handler that integrates with Remanence.
        ...
        if self._auto_recall and task:
            try:
                ...
                if matches:
                    ...
                    _log.info(
                        "RemanenceCallback: recalled %d patterns for task %r",
                        len(context),
                        task[:80],
                    )
                ...
                _log.warning("RemanenceCallback recall failed: %s", exc)
    ...
    def on_chain_end(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            _log.info("RemanenceCallback: learned from chain run for task %r", task[:80])
        except Exception as exc:
            _log.warning("RemanenceCallback learn failed: %s", exc)
