"""FastAPI application factory for Remanence.

Configuration is entirely via environment variables:

    REMANENCE_STORAGE        json | postgres          (default: json)
    REMANENCE_DATA_PATH      ./brain_data             (json only)
    REMANENCE_DATABASE_URL   postgresql://...         (postgres only)
    REMANENCE_LLM_PROVIDER   openai                   (default: openai)
    REMANENCE_LLM_MODEL      gpt-4.1                  (default: gpt-4.1)
    OPENAI_API_KEY       sk-...
    REMANENCE_EMBEDDING_MODEL text-embedding-3-small  (default)
    REMANENCE_API_KEYS       key1,key2                (empty = dev mode, no auth)
    REMANENCE_HOST           0.0.0.0                  (default)
    REMANENCE_PORT           8000                     (default)

Security configuration (env vars):
    REMANENCE_CORS_ORIGINS       comma-separated allowed origins (default: none — CORS disabled)
    REMANENCE_RATE_LIMIT_DEFAULT requests/min for regular endpoints (default: 60)
    REMANENCE_RATE_LIMIT_EXPENSIVE requests/min for LLM endpoints (default: 10)
    REMANENCE_MAX_BODY_SIZE      max request body in bytes (default: 1048576 = 1MB)

Run:
    uvicorn remanence.api.app:create_app --factory
...
from remanence import Memory, __version__
from remanence._factory import make_embeddings, make_llm, make_storage
...
def _log_security_config() -> None:
    ...
    api_keys_set = bool(os.environ.get("REMANENCE_API_KEYS", "").strip())
    if not api_keys_set:
        _log.warning(
            "SECURITY WARNING: Running in dev mode — API is unauthenticated. "
            "Set REMANENCE_API_KEYS=key1,key2 to require Bearer token authentication."
        )
    else:
        _log.info("SECURITY: API authentication enabled.")

    cors_origins = os.environ.get("REMANENCE_CORS_ORIGINS", "")
    if not cors_origins.strip():
        ...
        _log.warning(
            ...
            "Set REMANENCE_CORS_ORIGINS=https://yourapp.example.com for production."
        )
    else:
        _log.info("SECURITY: CORS restricted to configured origins.")

    max_body = int(os.environ.get("REMANENCE_MAX_BODY_SIZE", str(1024 * 1024)))
    rate_default = int(os.environ.get("REMANENCE_RATE_LIMIT_DEFAULT", "60"))
    rate_expensive = int(os.environ.get("REMANENCE_RATE_LIMIT_EXPENSIVE", "10"))
    _log.info(
...
def create_app() -> FastAPI:
    ...
    # ------------------------------------------------------------------
    from remanence.api.middleware import (
        BodySizeLimitMiddleware,
        RateLimitMiddleware,
        SecurityHeadersMiddleware,
    )

    cors_origins_raw = os.environ.get("REMANENCE_CORS_ORIGINS", "")
    ...
    _log.info(
        "Remanence API started — storage=%s, llm=%s",
...
# Uvicorn usage: uvicorn remanence.api.app:create_app --factory
# Or: from remanence.api.app import app  (triggers creation)
try:
    import os

    _skip_auto_create = os.environ.get("REMANENCE_SKIP_AUTO_APP", "0") == "1"
except Exception:
