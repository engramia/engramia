...
Provider selection (for recall):
    ...
    Set REMANENCE_LOCAL_EMBEDDINGS=1 to use local sentence-transformers (no API key).
"""
...
def _make_embeddings():
    ...
    if os.environ.get("REMANENCE_LOCAL_EMBEDDINGS"):
        try:
    ...
    if not api_key:
        console.print(
            ...
            "set [bold]REMANENCE_LOCAL_EMBEDDINGS=1[/bold] to use local sentence-transformers."
        )
        raise typer.Exit(1)
...
def init(
    ...
    """Initialize a new remanence data directory."""
    import pathlib
    ...
    console.print(f"[green]✓[/green] Initialized remanence data directory: [bold]{p.resolve()}[/bold]")
    console.print("\nNext steps:")
...
def serve(
    ...
    if storage == "json":
        os.environ.setdefault("REMANENCE_STORAGE", "json")
        os.environ.setdefault("REMANENCE_DATA_PATH", path)
    else:
        os.environ.setdefault("REMANENCE_STORAGE", storage)

    console.print(f"[green]Starting Remanence API[/green] on [bold]http://{host}:{port}[/bold]  (storage={storage})")
    console.print(f"  Swagger UI: [cyan]http://{host}:{port}/docs[/cyan]")

    uvicorn.run(
        "remanence.api.app:create_app",
        factory=True,
        host=host,
        port=port,
        reload=reload,
...
@app.command()
def status(
    path: str = typer.Option("./brain_data", "--path", "-p", help="remanence data directory."),
) -> None:
    ...
    table = Table(title=f"Remanence Status — {path}", show_header=True)
