...
Provider selection (for recall):
    ...
    Set BRAIN_LOCAL_EMBEDDINGS=1 to use local sentence-transformers (no API key).
"""
...
def _make_embeddings():
    ...
    if os.environ.get("BRAIN_LOCAL_EMBEDDINGS"):
        try:
    ...
    if not api_key:
        console.print(
            ...
            "set [bold]BRAIN_LOCAL_EMBEDDINGS=1[/bold] to use local sentence-transformers."
        )
        raise typer.Exit(1)
...
def init(
    ...
    console.print(f"[green]✓[/green] Initialized Brain data directory: [bold]{p.resolve()}[/bold]")
    console.print("\nNext steps:")
...
def serve(
    ...
    if storage == "json":
        os.environ.setdefault("BRAIN_STORAGE", "json")
        os.environ.setdefault("BRAIN_DATA_PATH", path)
    else:
    ...
    uvicorn.run(
        "agent_brain.api.app:create_app",
        factory=True,
        host=host,
        port=port,
        reload=reload,
...
@app.command()
def status(
    path: str = typer.Option("./brain_data", "--path", "-p", help="Brain data directory."),
) -> None:
    ...
    table = Table(title=f"Brain Status — {path}", show_header=True)
...
def recall(
    ...
    storage = _make_storage(path)
    brain = Brain(embeddings=embeddings, storage=storage)
