...
Generated: 2026-04-17 (interim — canonical regeneration scheduled for v0.6.5 release)  |  Engramia version: 0.6.5

> **⏳ Transitional state.** The package list below is the last audit
> snapshot before automation was introduced. It is legally complete
> (all licenses reviewed, no blocking issues) but does not yet include
> all runtime extras from [`pyproject.toml`](../../pyproject.toml)
> (notably `anthropic`, `mcp`, `cryptography`, `PyJWT`, `bcrypt`,
> `opentelemetry-*`, `stripe`, `alembic`, `langchain-core`, `crewai`).
> Those are audited transitively but have not been individually listed.
> The file will be fully canonicalized by
> [`scripts/audit_licenses.py`](../../scripts/audit_licenses.py) the
> next time [`prepare-release.yml`](../../.github/workflows/prepare-release.yml)
> runs (v0.6.5 release).

Runtime Python dependencies that ship with the `engramia` wheel and
Docker image. After the next release, this file will be auto-generated
by `scripts/audit_licenses.py` against a clean venv installed with
`pip install ".[all]"` (runtime extras only — no dev, docs, or test
tooling). Do not edit manually; CI will reject drift after
canonicalization.

Frontend dependencies (Next.js admin dashboard) ship in a separate
Docker image and are audited in the Dashboard repo:
[engramia/dashboard → docs/legal/DEPENDENCY_LICENSES.md](https://github.com/engramia/dashboard/blob/main/docs/legal/DEPENDENCY_LICENSES.md).
...
- **Release time** — [`prepare-release.yml`](../../.github/workflows/prepare-release.yml)
  installs the runtime extras into a clean venv and runs
  `scripts/audit_licenses.py`. The refreshed file is committed
  alongside the new LICENSE.txt before the release tag is pushed.
- **Pull requests** — `ci.yml` runs
  `python scripts/audit_licenses.py --check` whenever `pyproject.toml`
  changes, failing the build if the audit is stale.
- **Manual refresh** — `pip install ".[all]" pip-licenses` in a clean
  venv, then `python scripts/audit_licenses.py`.

---

*Auto-generated by `scripts/audit_licenses.py`. Do not edit manually.*