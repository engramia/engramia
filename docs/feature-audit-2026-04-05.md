...
|---|---|---|---|---|
| **Engramia** | 500 evals, 5K patterns | **$29/mo** | **$99/mo** | Custom |
| Mem0 | ~1K memories | ~$49/mo | ~$99/mo | Custom |
| Zep | Self-hosted | ~$49/mo | — | Custom |
| LangSmith | 5K traces/mo | $39/user/mo | — | Custom |
| Braintrust | Velmi omezený | ~$150/mo | — | Custom |
| LangFuse | 50K observations | $59/mo | ~$199/mo | Custom |
| Helicone | 10K req | $20/mo | ~$80/mo | Custom |
| DeepEval | OSS free | ~$59/mo | — | Custom |

**Klíčový závěr:** Engramia Pro ($29) je **nejlevnější paid entry** v celém segmentu a nabízí víc než jen tracing/observability (memory + eval v jednom). Toto je silná cenová pozice.

### Top 7 Universal Customer Pain Points (validace z trhu)

Tyto pain points jsou průběžně reportovány u všech competitors — a Engramia je buď řeší, nebo může řešit:

| Pain Point | Jak Engramia odpovídá |
|---|---|
| **Fragmentation** — 3-4 oddělené nástroje (memory, tracing, eval, prompts) | ✅ Unified: memory + eval + feedback + ROI analytics v jednom |
| **Memory opacity** — zákazník neví proč si agent zapamatoval X a zapomněl Y | ⚠️ Partial: `recall()` vrací similarity scores, ale admin dashboard nemá memory inspector UI |
| **LLM-judge unreliability** — jedno hodnocení je nespolehlivé | ✅ Multi-evaluator + variance detection řeší přesně toto |
| **Eval-production gap** — offline evaly nepredikují produkci | ✅ ROI analytics + composite score propojuje eval s business outcomes |
| **Free tier příliš omezený** — 5K LangSmith traces vyprší za hodiny | ✅ 500 eval runs / 5K patterns je rozumnější pro development |
| **Memory forgetting/decay** — kdykdo kontroluje co se zapomíná? | ✅ `run_aging()` s konfigurovatelným decay je unikátní feature |
| **Vendor lock-in** — data v proprietary formátu | ✅ `export()/import_data()` + self-hosted BSL option |

### Emerging Trends (2025-2026) relevantní pro Engramia

| Trend | Engramia status | Doporučení |
|---|---|---|
| **Procedural memory** (agenti si pamatují JAK dělat věci) | ✅ Přesně co Engramia dělá — success patterns = procedural memory | Lépe to komunikovat v marketingu |
| **Multi-agent shared memory** | ✅ Roadmap Phase 7 + Enterprise feature | Accelerate — competitors to nemají |
| **Memory regression testing** | ✅ Benchmark suite (254 tasks, 98.8%) | Exponovat jako standalone feature pro CI/CD |
| **Agent-specific evals** (ne jen RAG) | ✅ Core Engramia use case | Positioning win — Ragas/TruLens jsou RAG-only |
| **Red-teaming automation** | ⚠️ Adversarial detection existuje (hardcoded outputs), ale není generický | Rozšířit adversarial detection na Phase 2 |
| **Online eval** (production monitoring) | ✅ ROI analytics + Prometheus metrics | Dashboard pro real-time quality monitoring |
| **Memory evaluation metrics** | ❌ Nová sub-kategorie — kdo první, ten vyhraje | Přidat: memory precision/recall/staleness metrics |
