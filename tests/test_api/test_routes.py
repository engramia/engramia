...
class TestLearnEndpoint:
    def test_learn_returns_200(self, api_client):
        resp = api_client.post("/learn", json={
            "task": "Parse CSV file",
            "code": "import csv",
            "eval_score": 8.5,
        })
    ...
    def test_learn_increments_count(self, api_client):
        api_client.post("/learn", json={"task": "Task A", "code": "code_a", "eval_score": 7.0})
        api_client.post("/learn", json={"task": "Task B", "code": "code_b", "eval_score": 8.0})
        resp = api_client.post("/learn", json={"task": "Task C", "code": "code_c", "eval_score": 9.0})
...
class TestRecallEndpoint:
    def test_recall_returns_matches(self, api_client):
        api_client.post("/learn", json={"task": "Parse CSV file", "code": "import csv", "eval_score": 8.5})
        resp = api_client.post("/recall", json={"task": "Parse CSV file"})
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_recall_empty_store_returns_empty(self, api_client):
        resp = api_client.post("/recall", json={"task": "Read CSV data"})
        assert resp.status_code == 200
        assert resp.json()["matches"] == []

    def test_recall_match_has_pattern_key(self, api_client):
        api_client.post("/learn", json={"task": "Task", "code": "code", "eval_score": 7.0})
        resp = api_client.post("/recall", json={"task": "Task"})
    ...
    def test_recall_invalid_limit_returns_422(self, api_client):
        resp = api_client.post("/recall", json={"task": "task", "limit": 0})
        assert resp.status_code == 422
...
class TestEvaluateEndpoint:
    def test_evaluate_returns_median_score(self, api_client):
        resp = api_client.post("/evaluate", json={
...
class TestComposeEndpoint:
    def test_compose_returns_pipeline(self, api_client, mock_llm):
        ...
        api_client.post("/learn", json={"task": "Read CSV file", "code": "import csv", "eval_score": 8.0})
        resp = api_client.post("/compose", json={"task": "Read CSV and compute stats"})
        assert resp.status_code == 200
        data = resp.json()
...
class TestFeedbackEndpoint:
    ...
    def test_feedback_populated_after_evaluations(self, api_client):
        ...
        api_client.post("/evaluate", json={"task": "Parse CSV", "code": "import csv", "num_evals": 1})
        api_client.post("/evaluate", json={"task": "Parse CSV", "code": "import csv", "num_evals": 1})
        resp = api_client.get("/feedback")
...
class TestMetricsEndpoint:
    ...
    def test_metrics_increments_after_learn(self, api_client):
        api_client.post("/learn", json={"task": "Task", "code": "code", "eval_score": 8.0})
        resp = api_client.get("/metrics")
...
class TestDeletePatternEndpoint:
    def test_delete_existing_pattern(self, api_client):
        api_client.post("/learn", json={"task": "Task", "code": "code", "eval_score": 8.0})
        recall_resp = api_client.post("/recall", json={"task": "Task"})
        key = recall_resp.json()["matches"][0]["pattern_key"]

        del_resp = api_client.delete(f"/patterns/{key}")
        assert del_resp.status_code == 200
        assert del_resp.json()["deleted"] is True

        # Pattern should be gone
        recall_after = api_client.post("/recall", json={"task": "Task"})
        assert recall_after.json()["matches"] == []

    def test_delete_nonexistent_returns_false(self, api_client):
        resp = api_client.delete("/patterns/patterns/nonexistent_0000")
        assert resp.status_code == 200
