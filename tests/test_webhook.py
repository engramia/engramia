"""Unit tests for BrainWebhook — mocks urllib to avoid live HTTP calls."""
...
def _hook(response_body: dict) -> BrainWebhook:
    """Return a BrainWebhook whose HTTP calls always return *response_body*."""
    hook = BrainWebhook(url="http://brain.local", api_key="sk-test")
...
class TestErrorHandling:
    def test_http_error_raises_webhook_error(self):
        ...
        with patch("urllib.request.urlopen", side_effect=exc), pytest.raises(BrainWebhookError) as exc_info:
    ...
    def test_connection_error_raises_webhook_error(self):
        ...
        hook = BrainWebhook(url="http://brain.local")
        with (
            patch("urllib.request.urlopen", side_effect=urllib.error.URLError("Connection refused")),
            pytest.raises(BrainWebhookError) as exc_info,
        ):
            hook.health()
