...
def _make_callback(mem, **kwargs):
    ...
    with patch.dict("sys.modules", {"crewai": mock_crewai}):
        ...
        return EngramiaCrewCallback(mem, **kwargs)


class TestEngramiaCrewCallbackLearn:
    def test_task_callback_learns_from_output(self, mem):
        cb = _make_callback(mem, auto_learn=True, auto_recall=False)
    ...
    def test_task_callback_disabled_does_not_learn(self, mem):
        cb = _make_callback(mem, auto_learn=False, auto_recall=False)
        count_before = mem.metrics.pattern_count
    ...
    def test_task_callback_skips_empty_output(self, mem):
        cb = _make_callback(mem, auto_learn=True, auto_recall=False)
        count_before = mem.metrics.pattern_count
    ...
    def test_task_callback_skips_empty_task(self, mem):
        cb = _make_callback(mem, auto_learn=True, auto_recall=False)
        count_before = mem.metrics.pattern_count
    ...
    def test_task_callback_uses_default_score(self, mem):
        cb = _make_callback(mem, auto_learn=True, default_score=9.0)
        mem.learn(task="CSV parsing task", code="import csv", eval_score=5.0)
    ...
    def test_learn_failure_does_not_raise(self, mem):
        cb = _make_callback(mem, auto_learn=True)
        cb._brain = MagicMock()
...
class TestEngramiaCrewCallbackRecall:
    def test_inject_recall_appends_context(self, mem):
        mem.learn(task="Parse CSV file", code="import csv", eval_score=8.0)

        cb = _make_callback(mem, auto_recall=True, auto_learn=False)
    ...
    def test_inject_recall_skips_when_disabled(self, mem):
        mem.learn(task="Parse CSV file", code="import csv", eval_score=8.0)

        cb = _make_callback(mem, auto_recall=False, auto_learn=False)
    ...
    def test_inject_recall_no_matches_leaves_description_unchanged(self, mem):
        cb = _make_callback(mem, auto_recall=True, auto_learn=False)
    ...
    def test_inject_recall_multiple_tasks(self, mem):
        mem.learn(task="Fetch stock data", code="import yfinance", eval_score=7.0)
        mem.learn(task="Write markdown report", code="# Report\n...", eval_score=8.0)
    ...
    def test_inject_recall_handles_frozen_model(self, mem):
        mem.learn(task="CSV task", code="import csv", eval_score=8.0)

        cb = _make_callback(mem, auto_recall=True)
    ...
    def test_inject_recall_failure_does_not_raise(self, mem):
        cb = _make_callback(mem, auto_recall=True)
        cb._brain = MagicMock()
...
class TestEngramiaCrewCallbackKickoff:
    def test_kickoff_calls_crew_kickoff(self, mem):
        cb = _make_callback(mem, auto_learn=False, auto_recall=False)
    ...
    def test_kickoff_passes_inputs(self, mem):
        cb = _make_callback(mem, auto_learn=False, auto_recall=False)
    ...
    def test_kickoff_injects_recall_when_enabled(self, mem):
        mem.learn(task="Parse CSV file", code="import csv", eval_score=8.0)

        cb = _make_callback(mem, auto_learn=False, auto_recall=True)
...
class TestEngramiaCrewCallbackHelpers:
    def test_get_learned_count(self, mem):
        cb = _make_callback(mem, auto_learn=True)
        assert cb.get_learned_count() == 0

        mem.learn(task="Something", code="code", eval_score=7.0)
