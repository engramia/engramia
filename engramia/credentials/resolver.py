...
class CredentialResolver:
    ...
    Args:
        ...
        cipher: :class:`AESGCMCipher` configured with the operator's master
            key. None disables resolution (forces all callers to demo mode);
            useful in dev when ``ENGRAMIA_BYOK_ENABLED=false``.
    ...
    def __init__(self, store: CredentialStore, cipher: AESGCMCipher | None) -> None:
        self._store = store
        self._cipher = cipher
    ...
    def resolve(
        ...
        try:
            ...
            _log.warning(
                "CREDENTIAL_DECRYPT_FAILURE row_id=%s tenant=%s provider=%s — marking invalid",
                row.id,
                tenant_id,
                row.provider,
            )
    ...
    def _decrypt(self, row: StoredCredential) -> str:
        """Decrypt ``row.encrypted_key`` using AAD derived from the row.

        AAD is bound to the **stored** ``(tenant_id, provider, purpose)``
        triple, matching what the create path used at encryption time.
        Swapping a row to a different tenant in the DB makes this AAD
        differ from what the cipher was given at encrypt — the GCM tag
        check fails and ``DecryptionError`` is raised.
        """
        assert self._cipher is not None  # checked at resolve()
        aad = f"{row.tenant_id}:{row.provider}:{row.purpose}".encode()
        return self._cipher.decrypt(row.encrypted_key, row.nonce, row.auth_tag, aad)
