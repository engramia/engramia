...
class EngramiaCallback:
    """LangChain callback handler that integrates with Engramia.
        ...
        if self._auto_recall and task:
            try:
                ...
                if matches:
                    ...
                    _log.info(
                        "EngramiaCallback: recalled %d patterns for task %r",
                        len(context),
                        task[:80],
                    )
                ...
                _log.warning("EngramiaCallback recall failed: %s", exc)
    ...
    def on_chain_end(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            _log.info("EngramiaCallback: learned from chain run for task %r", task[:80])
        except Exception as exc:
            _log.warning("EngramiaCallback learn failed: %s", exc)
